...
public class ValueCounter {
    ...
    private String[] lblBMI = {"Underweight", "Normal", "Overweight", "Obese"};
    private int[] valBMI = {0, 0, 0, 0};
    ...
    private String calculateBMI(PatientRecord patientRecord) {
        int height, weight;
        height = new Double(patientRecord.getRecord().getHeight()).intValue();
        weight = new Double(patientRecord.getRecord().getWeight()).intValue();
        float bmi = BMICalculator.computeBMIMetric(height, weight);
        String result = BMICalculator.getBMIResultString(patientRecord.getGender(), patientRecord.getAge(), bmi);
        return result;
    }
